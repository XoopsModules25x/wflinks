<?php
/**
 * $Id: singlelink.php 9722 2012-06-26 02:30:10Z beckmi $
 * Module: WF-Links
 * Version: v1.0.3
...
$xoopsOption['template_main'] = 'wflinks_singlelink.html';
...
if ( $xoopsModuleConfig['useaddress'] == 1 ) {
  ...
  if ( $link_arr['tel'] == true ) {
    $link['tel'] = '<br />' . '<img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/images/icon/telephone.png" title="'._MD_WFL_TELEPHONE.'" alt="'._MD_WFL_TELEPHONE.'" align="absmiddle" />&nbsp;' . $tel;
  }

  if ( $link_arr['mobile'] == true ) {
    $link['mobile'] = '<br />' . '<img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/images/icon/phone.png" title="'._MD_WFL_MOBILE.'" alt="'._MD_WFL_MOBILE.'" align="absmiddle" />&nbsp;' . $mobile;
  }

  if ( $link_arr['voip'] == true ) {
    $link['voip'] = '<br />' .'<img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/images/icon/voip.png" title="'._MD_WFL_VOIP.'" alt="'._MD_WFL_VOIP.'" align="absmiddle" />&nbsp;'  . $voip;
  }

  if ( $link_arr['fax'] == true ) {
    $link['fax'] = '<br />' . '<img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/images/icon/fax.png" title="'._MD_WFL_FAX.'" alt="'._MD_WFL_FAX.'" align="absmiddle" />&nbsp;' . $fax;
  }

  if ( $link_arr['email'] == true ) {
    $link['email'] = '<br />' . '<img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/images/icon/email.png" title="'._MD_WFL_EMAIL . '" alt="' ._MD_WFL_EMAIL.'" align="absmiddle" />&nbsp;' . $email;
  }
...
if ( $link_arr['street1'] == true || $link_arr['tel'] == true || $link_arr['mobile'] == true || $link_arr['fax'] == true || $link_arr['email'] == true ) {
  ...
  $xoopsTpl -> assign( 'vcard' , '<br />' . '<a href="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/vcard.php?lid=' . $link_arr['lid'] .'"><img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getvar( 'dirname' ) . '/images/icon/vcard.png" title="vCard" alt="vCard" /></a>' );
}
...
if ( $xoopsModuleConfig['useaddress'] == 1 ) {
  ...
  if ( $link_arr['googlemap'] == true) {
    $link['googlemap'] = '<a href="' . $googlemap . '" target="_blank"><img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getVar( 'dirname' ) . '/images/icon/google_map.png" alt="' . _MD_WFL_GETMAP . '" title="' . _MD_WFL_GETMAP . '" align="absmiddle"/></a>&nbsp;';
  }

  if ( $link_arr['yahoomap'] == true) {
    $link['yahoomap'] = '<a href="' . $yahoomap . '" target="_blank"><img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getVar( 'dirname' ) . '/images/icon/yahoo_map.png" alt="' . _MD_WFL_GETMAP . '" title="' . _MD_WFL_GETMAP . '" align="absmiddle"/></a>&nbsp;';
  }

  if ( $link_arr['multimap'] == true) {
    $link['multimap'] = '<a href="' . $multimap . '" target="_blank"><img src="' . XOOPS_URL . '/modules/' . $xoopsModule -> getVar( 'dirname' ) . '/images/icon/multimap.png" alt="' . _MD_WFL_GETMAP . '" title="' . _MD_WFL_GETMAP . '" align="absmiddle"/></a>';
  }
}
