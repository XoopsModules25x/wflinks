...
<table border="0" cellpadding="1" cellspacing="2" width="80%" align="center">
  ...
  <tr>
    <td align="center">
	  <div><b><{$smarty.const._MD_WFL_VOTEFORTITLE}>: <{$link.title}></b></div><br />
      <form method="post" action="ratelink.php">
        ...
        <input type="submit" name="submit" value="<{$smarty.const._MD_WFL_RATEIT}>" alt="<{$smarty.const._MD_WFL_RATEIT}>" /> <input type="button" value="<{$smarty.const._CANCEL}>" alt="<{$smarty.const._CANCEL}>" onclick="location='<{$xoops_url}>/modules/<{$module_dir}>/singlelink.php?cid=<{$link.cid}>&amp;lid=<{$link.id}>'" />
      </form>
    </td>
  </tr>
</table>
