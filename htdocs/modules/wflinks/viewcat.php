...
$sql = "SELECT title, description, nohtml, nosmiley, noxcodes, noimages, nobreak, imgurl, client_id, banner_id FROM " . $xoopsDB -> prefix( 'wflinks_cat' ) . " WHERE cid =" . intval( $cid );
...
if ( $selectdate ) {
    ...
    $sql = "SELECT DISTINCT a.* FROM " . $xoopsDB -> prefix( 'wflinks_links' ) . " a LEFT JOIN "
    ...
    $sql2 = "SELECT COUNT(*) FROM " . $xoopsDB -> prefix( 'wflinks_links' ) . " a LEFT JOIN " . $xoopsDB -> prefix( 'wflinks_altcat' ) . " b "
     . " ON b.lid = a.lid"
     . " WHERE a.published > 0 AND a.published <= " . $time
     . " AND (a.expired = 0 OR a.expired > " . $time . ") AND a.offline = 0"
     . " AND (b.cid=a.cid OR (a.cid=" . intval($cid) . " OR b.cid=" . intval($cid) . "))";
